...
from .const import CONF_PASSWORD, CONF_TENANT_ID, CONF_USERNAME, DOMAIN, PLATFORMS
...
async def async_setup_entry(hass: HomeAssistant, entry: SomtodayConfigEntry) -> bool:
    ...
    client = SomtodayClient(
        async_get_clientsession(hass), entry.data[CONF_TENANT_ID], entry.data[CONF_USERNAME], entry.data[CONF_PASSWORD]
    )
